...
namespace stromx
{
    ...
    namespace cvimgutil
    {
        ...
        const std::string ConstImage::PACKAGE(STROMX_IMGUTIL_PACKAGE_NAME);
        ...
        void ConstImage::execute(DataProvider& provider)
        {
            ...
            if(! outData)
                outData = new cvimgutil::Image(m_image->width(), m_image->height(), m_image->pixelType());
            
            cvimgutil::Image* outImage = reinterpret_cast<cvimgutil::Image*>(outData);
