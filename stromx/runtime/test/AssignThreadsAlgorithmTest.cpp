...
namespace stromx
{
    namespace runtime
    {
        ...
        void AssignThreadsAlgorithmTest::testApplyWrongOrder()
        {
            m_stream = new Stream;
        ...
        void AssignThreadsAlgorithmTest::testApplyParallel()
        {
            m_stream = new Stream;
            ...
            CPPUNIT_ASSERT_EQUAL(std::size_t(1), thread1->inputSequence().size());
            CPPUNIT_ASSERT_EQUAL(std::size_t(1), thread2->inputSequence().size());
            CPPUNIT_ASSERT_EQUAL(std::size_t(2), thread3->inputSequence().size());
        }
        
        void AssignThreadsAlgorithmTest::testTwoOutputsAtOneInput()
        {
            m_stream = new Stream;
            ...
            m_stream->connect(counter, Counter::OUTPUT, dump0, Dump::INPUT);
            m_stream->connect(counter, Counter::OUTPUT, dump1, Dump::INPUT);
