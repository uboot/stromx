...
namespace stromx
{
    namespace runtime
    {
        const std::string IsNotEmpty::TITLE("IsNotEmpty");
        ...
        Data* IsNotEmpty::getProperty(const Matrix & matrix)
        {
            return new Bool(matrix.rows() != 0 && matrix.cols() != 0);
        }
        
        Output* IsNotEmpty::createOutput()
        {
            Output* output = new Output(PROPERTY, Variant::BOOL);
            output->setTitle(L_("Is not empty"));
            return output;
        }
    } 
}
